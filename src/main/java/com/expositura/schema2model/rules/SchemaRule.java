...
import com.expositura.schema2model.Schema2Model;
...
public class SchemaRule implements Rule<JClassContainer, JType> {
   ...
   * <p>
   * At the root of a schema document this rule should be applied (schema documents contain a schema), but also in many
   * places within the document. Each property of type "object" is itself defined by a schema, the items attribute of an
   * array is a schema, the additionalProperties attribute of a schema is also a schema.
   * <p>
  ...
  private String nameFromRef(String ref) {
    ...
    String nameFromRef;
    if (!contains(ref, "#")) {
      nameFromRef = Schema2Model.getNodeName(ref, ruleFactory.getGenerationConfig());
    } else {
