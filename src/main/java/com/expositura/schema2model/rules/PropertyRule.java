...
public class PropertyRule implements Rule<JDefinedClass, JDefinedClass> {
   ...
   * <p>
   * This rule adds a property to a given Java class according to the Java Bean spec. A private field is added to the
   * class, along with accompanying accessor methods.
   * <p>
   * If this rule's schema mapper is configured to include builder methods (see
   * {@link GenerationConfig#isGenerateBuilders()} ), then a builder method of the form <code>withFoo(Foo foo);</code>
   * is also added.
   *
  ...
  private JMethod addSetter(JDefinedClass c, JFieldVar field, String jsonPropertyName, JsonNode node) {
    ...
    JVar param = setter.param(JMod.FINAL, field.type(), field.name());
