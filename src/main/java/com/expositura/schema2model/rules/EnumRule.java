...
import com.expositura.schema2model.Annotator;
import com.expositura.schema2model.RuleLogger;
import com.expositura.schema2model.Schema;
import com.expositura.schema2model.exception.ClassAlreadyExistsException;
import com.expositura.schema2model.exception.GenerationException;
import com.expositura.schema2model.model.EnumDefinition;
import com.expositura.schema2model.model.EnumDefinitionExtensionType;
import com.expositura.schema2model.model.EnumValueDefinition;
import com.expositura.schema2model.util.AnnotationHelper;
import static java.util.Arrays.*;
import static org.apache.commons.lang3.StringUtils.*;
import static com.expositura.schema2model.rules.PrimitiveTypes.*;
import static com.expositura.schema2model.util.TypeUtil.*;
...
public class EnumRule implements Rule<JClassContainer, JType> {
   ...
   * <p>
   * A Java {@link Enum} is created, with constants for each of the enum values present in the schema. The enum name is
   * derived from the nodeName, and the enum type itself is created as an inner class of the owning type. In the rare
   * case that no owning type exists (the enum is the root of the schema), then the enum becomes a public class in its
   * own right.
   * <p>
   * The actual JSON value for each enum constant is held in a property called "value" in the generated type. A static
   * factory method <code>fromValue(String)</code> is added to the generated enum, and the methods are annotated to
   * allow Jackson to marshal/unmarshal values correctly.
   *
   ...
   *
   * For "enum" handled by {@link #buildEnumDefinitionWithNoExtensions(String, JsonNode, JsonNode, JType)} For "enum"
   * and "javaEnums" handled by
   * {@link #buildEnumDefinitionWithJavaEnumsExtension(String, JsonNode, JsonNode, JsonNode, JType)} For "enum" and
   * "javaEnumNames" handled by
   * {@link #buildEnumDefinitionWithJavaEnumNamesExtension(String, JsonNode, JsonNode, JsonNode, JType)}
   *
   * @param nodeName the name of the property which is an "enum"
   * @param node the enum node
   * @param backingType the object backing the value of enum, most commonly this is a string
   *
