...
public class AdditionalPropertiesRule implements Rule<JDefinedClass, JDefinedClass> {
   ...
   * <p>
   * If additionalProperties is specified and set to the boolean value <code>false</code>, this rule does not make any
   * change to the generated Java type (the type does not allow additional properties).
   * <p>
   * If the additionalProperties node is <code>null</code> (not specified in the schema) or empty, then a new bean
   * property named "additionalProperties", of type {@link Map}{@literal <String,Object>} is added to the generated type
   * (with appropriate accessors). The accessors are annotated to allow unrecognised (additional) properties found in
   * JSON data to be marshalled/unmarshalled from/to this map.
   * <p>
   * If the additionalProperties node is present and specifies a schema, then an "additionalProperties" map is added to
   * the generated type. This time the map values will be restricted and must be instances of a newly generated Java
   * type that will be created based on the additionalProperties schema provided. If the schema does not specify the
   * javaType property, the name of the newly generated type will be derived from the nodeName and the suffix
   * 'Property'.
   *
