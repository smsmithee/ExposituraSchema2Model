...
import com.expositura.schema2model.URLProtocol;
import java.io.File;
...
public class URLUtil {
  ...
  public static URL parseURL(String input) {
    try {
      return switch (parseProtocol(input)) {
        case NO_PROTOCOL -> new File(input).toURI().toURL();
        default -> URI.create(input).toURL();
      };
