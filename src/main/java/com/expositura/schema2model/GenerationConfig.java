...
/**
 * Defines the configuration options for the maven plugin.
 */
public interface GenerationConfig {
   ...
   *
   * @return Whether to generate builder-style methods of the form <code>withXxx(value)</code> (that return
   ...
   *
   * @see <a href="https://github.com/FasterXML/jackson-docs/wiki/JacksonPolymorphicDeserialization">Jackson Polymorphic
   * Deserialization</a>
   *
   */
   ...
   *
   * @return whether to include JDK Constructor Properties. Used by serialization libraries to get parameter names of
   * constructors at runtime. (Not Available on Android)
   *
   * @see
   * <a href="https://docs.oracle.com/javase/7/docs/api/java/beans/ConstructorProperties.html">ConstructorProperties</a>
   */
   ...
   * wrapper types where possible when generating bean properties (has the side-effect of making those properties
   * non-null).
   */
   ...
   *
   * @return Whether to use the java type <code>long</code> (or {@link java.lang.Long}) instead of <code>int</code> (or
   * {@link java.lang.Integer}) when representing the JSON Schema type 'integer'.
   */
   ...
   *
   * @return Whether to use the java type {@link java.math.BigInteger} instead of <code>int</code> (or
   ...
   *
   * @return Whether to use the java type <code>double</code> (or {@link java.lang.Double}) instead of
   * <code>float</code> (or {@link java.lang.Float}) when representing the JSON Schema type 'number'.
   */
   ...
   *
   * @return Whether to use the java type {@link java.math.BigDecimal} instead of <code>float</code> (or
   ...
   *
   * @return An array of strings representing fields that should be excluded from toString methods
   */
   ...
   * annotations (for schema rules like minimum, maximum, etc) in generated Java types.
   */
   ...
   * annotations (for schema rules like Nullable, NonNull, etc) in generated Java types.
   */
   ...
   *
   * @return Whether to use {@link java.util.Optional} as return type for getters of non-required fields.
   */
   ...
   * been generated previously. <strong>Be warned</strong>, when activated this option will cause schema2model to
   * <strong>indiscriminately delete the entire contents of the target directory (all files and folders)</strong> before
   * it begins generating sources.
   */
   ...
   *
   * @return Whether to use {@link org.joda.time.LocalDate} instead of string when adding string type fields with a
   * format of date (not date-time) to generated Java types.
   */
   ...
   *
   * @return Whether to use {@link org.joda.time.LocalTime} instead of string when adding string type fields with a
   * format of time (not date-time) to generated Java types.
   */
  ...
  /**
   * Gets the 'constructorsRequiredPropertiesOnly' configuration option. This is a legacy configuration option used to
   * turn on the {@link #isIncludeRequiredPropertiesConstructor()} and off the
   * {@link #isIncludeAllPropertiesConstructor()} configuration options. It is specifically tied to the
   * {@link #isIncludeConstructors()} property, and will do nothing if that property is not enabled
   *
   * @return whether the resulting object should have a constructor with only the required properties as parameters or
   * all properties.
   */
   ...
   * set to true.
   *
   * @return whether the resulting object should include a constructor the class itself as a parameter, with the
   * expectation that all properties from the originating class will assigned to the new class.
   */
   ...
   *
   * @return Whether to allow 'additional properties' support in objects. Setting this to false will disable additional
   ...
   *
   * @return Whether to include setters or to omit these accessor methods and create public fields instead.
   */
   ...
   *
   * @return Whether to include dynamic getters, setters, and builders or to omit these methods.
   */
   ...
   * </ul>
   *
   * @return The java type to use instead of string when adding string type fields with a format of date (not date-time)
   ...
   * </ul>
   *
   * @return The java type to use instead of string when adding string type fields with a format of time (not date-time)
   * to generated Java types.
   */
  String getTimeType();
   ...
   *
   * @return Whether the fields of type <code>date</code> have the <code>@JsonFormat</code> annotation with pattern set
   * to the default value of <code>yyyy-MM-dd</code>
   */
   ...
   *
   * @return Whether the fields of type <code>time</code> have the <code>@JsonFormat</code> annotation with pattern set
   * to the default value of <code>HH:mm:ss.SSS</code>
   */
   ...
   *
   * @return Whether the fields of type <code>date-type</code> have the <code>@JsonFormat</code> annotation with pattern
   * set to the default value of <code>yyyy-MM-dd'T'HH:mm:ss.SSSZ</code>
   */
   ...
   *
   * @return The custom format that dates will use when types are serialized. Requires support from your JSON binding
   * library.
   */
   ...
   *
   * @return The custom format that times will use when types are serialized. Requires support from your JSON binding
   * library.
   */
   ...
   *
   * @return A string containing any characters that should act as path delimiters when resolving $ref fragments. By
   * default, #, / and . are used in an attempt to support JSON Pointer and JSON Path.
   */
   ...
   *
   * @return An optional mapping from format identifier (e.g. 'uri') to fully qualified type name (e.g. 'java.net.URI').
   */
   ...
   *
   * @return whether to include the gang of four builder patter on the generated classes. The default value for this is
   * false.
   */
