...
public enum SourceSortOrder {
  ...
  /**
   * <code>FILES_FIRST</code> Case sensitive sort, visit the files first. The source files are processed in a breadth
   * first sort order.
   */
  FILES_FIRST {
    @Override
    public Comparator<File> getComparator() {
      return (final File fileA, final File fileB) -> {
  ...
  /**
   * <code>SUBDIRS_FIRST</code> Case sensitive sort, visit the sub-directories before the files. The source files are
