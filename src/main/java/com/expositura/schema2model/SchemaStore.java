...
public class SchemaStore {
  ...
  /**
   * Create or look up a new schema using the given schema as a parent and the path as a relative reference. If a schema
   * with the given parent and relative path is already known, then a reference to the original schema will be returned.
   *
   * @param parent the schema which is the parent of the schema to be created.
   * @param path the relative path of this schema (will be used to create a complete URI by resolving this path against
   * the parent schema's id)
   * @param refFragmentPathDelimiters A string containing any characters that should act as path delimiters when
   * resolving $ref fragments.
   * @return a schema object containing the contents of the given path
   */
